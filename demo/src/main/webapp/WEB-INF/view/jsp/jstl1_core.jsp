...
<body style="margin: 40px;">
	<h1>JavaServer Pages Standard Tag Library (JSTL)</h1>
	<hr>
		<table border="1">
		...
		<table border="1">
			...
			<tr>
				<td>${loop.index}</td>
				<td>	<%-- index가 홀수면 purple, 짝수면 pink --%>
					<span style="color: ${loop.count mod 2 eq 1 ? 'purple' : 'pink'};">${loop.count}</span>
				</td>
				<td>	<%-- ID가 홀수면 blue, 짝수면 red --%>
				<c:if test="${member.id mod 2 eq 1}">	<%-- if(member.id % 2 == 1) --%>
					<span style="color: blue;">${member.id}</span>
				</c:if>
				<c:if test="${member.id mod 2 eq 0}">	<%-- if(member.id % 2 == 1) --%>
					<span style="color: red;">${member.id}</span>
				</c:if>	
				</td>
				<td>	<%-- 이름 첫번째 줄 배경색은 yellow, 마지막줄은 cyan --%>
				...
				<td>	<%-- 한국이면 배경색을 orange, 미국이면 green --%>
				...
				<c:if test="${member.addr.country eq '미국'}">
					<span style="background-color: green;">${member.addr}</span>
				</c:if>
